...
from sqlalchemy import mapper, relation, select, object_session

from channelguide import db
from channelguide import util
from channelguide.auth import tables, AuthError
...
class User(UserBase):
    ...
    def add_moderator_action(self, channel, action):
        connection = object_session(self).connection(user_mapper)
        connection.execute(tables.moderator_action.insert(), user_id=self.id,
                channel_id=channel.id, action=action)

user_select = select([tables.user,
    util.aggregate_subquery('moderator_action_count',
        util.count_distinct(tables.moderator_action.c.channel_id),
        tables.moderator_action),
    ])
user_mapper = mapper(User, user_select.alias())
